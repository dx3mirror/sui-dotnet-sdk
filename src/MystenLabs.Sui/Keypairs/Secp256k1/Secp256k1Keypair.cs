...
/// <summary>
/// Default BIP-32 derivation path for Secp256k1 (Sui).
/// </summary>
public static class Secp256k1Constants
{
    public const string DefaultDerivationPath = "m/54'/784'/0'/0/0";
}

/// <summary>
/// Secp256k1 (K-256) keypair for signing Sui transactions and messages.
/// </summary>
public sealed class Secp256k1Keypair : Keypair
{
    ...
    /// </summary>
    public Secp256k1Keypair(byte[] secretKey)
    {
        ...
        _publicKey = Secp256k1Impl.GetPublicKey(_secretKey);
    }

    /// <summary>
    /// Creates a keypair from a Sui Bech32 private key string (suiprivkey1...).
    /// </summary>
    public static Secp256k1Keypair FromSecretKey(string bech32)
    {
        (SignatureScheme scheme, byte[] secretKey) = SuiPrivateKeyEncoding.Decode(bech32);
        if (scheme != SignatureScheme.Secp256k1)
        {
            throw new ArgumentException($"Expected Secp256k1 key, got {scheme}.", nameof(bech32));
        }
    ...
    /// </summary>
    public static Secp256k1Keypair FromSecretKey(byte[] secretKey, bool skipValidation = false)
    {
        if (secretKey == null || secretKey.Length != PrivateKeySizeBytes)
        {
            throw new ArgumentException(
        ...
        if (!skipValidation)
        {
            ...
            byte[] signature = Secp256k1Impl.Sign(secretKey, digest);
            byte[] publicKey = Secp256k1Impl.GetPublicKey(secretKey);
            if (!Secp256k1Impl.Verify(signature, digest, publicKey))
            {
    ...
    /// <summary>
    /// Derives a Secp256k1 keypair from a mnemonic phrase and optional BIP-32 path.
    /// Path must be m/54'/784'/{account}'/{change}/{address}.
    /// </summary>
    /// <param name="mnemonic">Space-separated mnemonic words (e.g. 12 words).</param>
    /// <param name="path">Optional path; defaults to <see cref="Secp256k1Constants.DefaultDerivationPath"/>.</param>
    /// <returns>Derived keypair.</returns>
    public static Secp256k1Keypair DeriveKeypair(string mnemonic, string? path = null)
    {
        string derivationPath = path ?? Secp256k1Constants.DefaultDerivationPath;
    ...
    {
        return Secp256k1Impl.Sign(_secretKey, digest);
    }
    ...
    {
        return new Secp256k1PublicKey(_publicKey);
    }

    /// <inheritdoc />
    public override string GetSecretKey()
    {
        return SuiPrivateKeyEncoding.Encode(_secretKey, SignatureScheme.Secp256k1);
    }
}
